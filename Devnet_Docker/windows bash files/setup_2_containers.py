...
with open("devnet_2x1x3/Devnet-1x3-Two/Network_1x3-2.yml", "r") as f:
    contents = f.read()
    contents = contents.replace("ENV_PEER_ADDY", peer_addy)
    contents = contents.replace("ENV_BOOTNODE_ADDY", bootnode_addy)

with open("devnet_2x1x3/Devnet-1x3-Two/Network_1x3-2.yml", "w") as f:
    f.write(contents)

# Copy the genesis.json file from the Devnet-1x3-One/execution directory to the Devnet-1x3-Two/execution directory
shutil.copy("devnet_2x1x3/Devnet-1x3-One/execution/genesis.json", "devnet_2x1x3/Devnet-1x3-Two/execution/")

# Start the Docker containers for the Devnet-1x3-Two network in the background
subprocess.run(["docker-compose", "-f", "devnet_2x1x3/Devnet-1x3-Two/Network_1x3-2.yml", "up", "-d"])