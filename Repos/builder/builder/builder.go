...
func (b *Builder) Start() error {
	...
	go func() {
		...
		log.Debug("==================== Starting builder for loop =====================")
		for {
			log.Debug("==================== Inside builder forloop =====================")
			select {
			case <-b.stop:
				log.Debug("==================== B has stopped =====================")
				return
			case payloadAttributes := <-c:
				// Right now we are building only on a single head. This might change in the future!
				log.Debug("==================== Starting Slot check =====================")
				if payloadAttributes.Slot < currentSlot {
					log.Debug("==================== Event slot too old =====================")
					continue
				} else if payloadAttributes.Slot == currentSlot {
					log.Debug("==================== Event slot exactly onpoint =====================")
					...
					log.Debug("==================== Event slot in the future =====================")
...
func (b *Builder) submitCapellaBlock(block *types.Block, blockValue *big.Int, ordersClosedAt, sealedAt time.Time,
	...
	if b.dryRun {
		log.Debug("=========================================THIS IS A DRY RUN MF=====================================================")
