...
func (r *RemoteRelay) GetValidatorForSlot(nextSlot uint64) (ValidatorData, error) {
	...
	log.Debug("=================NEXTSLOT: ===================", nextSlot)
	vd, found := r.validatorSlotMap[nextSlot]
	log.Debug("vd, Found ============ ", vd, found)
...
func (r *RemoteRelay) getSlotValidatorMapFromRelay() (map[uint64]ValidatorData, error) {
	...
	log.Debug("================== requested validators, resp code: ===================", code, dst)
	if err != nil {
		return nil, err
	}
	...
	for _, data := range dst {
		...
		log.Debug("================== val pk Hex ===================", strings.ToLower(data.Entry.Message.Pubkey), data.Slot)
